...
tf::Quaternion x_point(0.05,0,0,0);//向x方向每次运动的距离 可以调节此处更改灵敏度
tf::Quaternion y_point(0,0.05,0,0);//向y方向每次运动的距离

geometry_msgs::Point trans_x,trans_y;

geometry_msgs::Point transform_into_robot(geometry_msgs::Quaternion world_quaternion,tf::Quaternion point_tf){
    tf::Quaternion world_tf(world_quaternion.x,world_quaternion.y,world_quaternion.z,world_quaternion.w);
    tf::Quaternion tansformed_point = world_tf*point_tf*world_tf.inverse();
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber keyboard_sub = keyboard_sub_node.subscribe<std_msgs::Char>("keyboard_input_robot", 1,
