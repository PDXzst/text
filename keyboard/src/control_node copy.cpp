...
tf2::Quaternion Tf_euler2Quaternion(const double roll, const double pitch, const double yaw)  
{  
...
Euler_angle Tf_Quaterniond2Euler(const double x,const double y,const double z,const double w)
{
    
    double roll,pitch,yaw;
    tf2::Quaternion tf_q(x,y,z,w);
    tf2::Matrix3x3 m(tf_q);
    m.getRPY(roll, pitch, yaw);//转换
    Euler_angle angle;
    angle.roll = roll;
    angle.pitch = pitch;
    angle.yaw = yaw;
...
void keyboardCallback(const std_msgs::Char::ConstPtr &msg, geometry_msgs::PoseStamped &pose, const PX4_Control &px4_control, float target_altitude, ros::Publisher &targetPosPub, ros::Rate &loopRate, bool &control_pose_flag, bool &exit_flag)
{
    ...
    Euler_angle auv_eul = Tf_Quaterniond2Euler(pose.pose.orientation.x,pose.pose.orientation.y,pose.pose.orientation.z,pose.pose.orientation.w);
    ...
    tf2::Quaternion auv_q = Tf_euler2Quaternion(auv_eul.roll,auv_eul.pitch,auv_eul.yaw);
    pose.pose.orientation.x = auv_q.getX();
    pose.pose.orientation.y = auv_q.getY();
    pose.pose.orientation.z = auv_q.getZ();
    pose.pose.orientation.w = auv_q.getW();
}
